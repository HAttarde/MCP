...
with st.sidebar:
    ...
    with st.container():
        ...
        server_tools_options = ["", "sqlserver_crud", "postgresql_crud"]
        ...
        llm_index = llm_options.index("GPT-4o") if application == "MCP Application" else llm_options.index(st.session_state.get("llm_select", ""))
        server_tools_index = server_tools_options.index("sqlserver_crud") if application == "MCP Application" else server_tools_options.index(st.session_state.get("server_tools", ""))
...
if application == "MCP Application":
    ...
    TOOL_DESCRIPTIONS = """
    Available tools:
    1. sqlserver_crud: CRUD on Customers (SQL Server)
    2. postgresql_crud: CRUD on Products (PostgreSQL)
    """

    def _clean_json(raw: str) -> str:
        fences = re.findall(r"```(?:json)?([\s\S]*?)```", raw)
    ...
    def parse_user_query(query: str) -> dict:
        ...
        "If the user wants to see the columns, strcuture, or schema of a table, or says 'describe' or 'show columns', use action: 'describe' and set args ['table_name'].\n"
        ...
        "JSON: {\"tool\": \"sqlserver_crud\", \"action\": \"read\", \"args\": {}}\n"
    ...
    async def _invoke_tool(tool: str, action: str, args: dict) -> any:
        transport = StreamableHttpTransport(f"{MCP_SERVER_URL}/mcp")
    ...
    for msg in st.session_state.messages:
        if msg["role"] == "user":
            ...
            if action == "read" and isinstance(content["result"], list):
                ...
                df = pd.DataFrame(content['result'])
                st.table(df)
                st.markdown("This shows the column names, data types, nullability, and maximum length for each column in the table.")
        else:
            st.markdown(
                f"""
                <div class="chat-row left">
    ...
    if send_clicked and user_query_input:
            ...
            if tool == "sqlserver_crud" and action == "update":
                ...
                if "customer_id" not in args and "name" in args:
                    ...
                    if isinstance(read_result, dict) and "result" in read_result:
                        matches = [r for r in read_result["result"] if r.get("Name", "").lower() == args["name"].lower()]
                        if matches:
        ...
        # Try to extract from text if not found
                    ...
                    if extracted_name:
                        ...
                        read_result = call_mcp_tool(tool, "read", read_args)
                        if isinstance(read_result, dict) and "result" in read_result:
                            matches = [r for r in read_result["result"] if r.get("Name", "").lower() == extracted_name.lower()]
                            if matches:
                                args["customer_id"] = matches[0]["Id"]
                                p["args"] = args
        ...
        # Try to extract from text if not found
                    ...
                    if match:
                        ...
                        read_result = call_mcp_tool(tool, "read", read_args)
                        if isinstance(read_result, dict) and "result" in read_result:
                            matches = [r for r in read_result["result"] if r.get("name", "").lower() == product_name.lower()]
                            if matches:
                                args["product_id"] = matches[0]["id"]
                                p["args"] = args
